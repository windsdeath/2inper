---
type: folder-readme
status: substantial
confidence: high
owner: knowledge-manager
tags: [meta, moc, index, agent-entry-point]
created: 2026-04-25
last_reviewed: 2026-04-25
---

# Wiki Index

**모든 에이전트의 첫 진입점.** 새 작업을 받으면 여기를 먼저 읽고, 관련 community/concept/entity 페이지로만 추가 진입하세요. `graph.json` 직접 파싱은 최후의 수단.

이 파일은 Graphify의 `--wiki` 출력으로 부분 자동 갱신되며, 빈 섹션은 [[skills/knowledge-manager/SKILL|Knowledge Manager Agent]]가 채웁니다.

## 회사 정체성

- [[wiki/concepts/2INPER 미션]]
- [[wiki/concepts/브랜드 보이스]]
- [[wiki/concepts/디자인 시스템]]
- [[wiki/concepts/타겟 고객]]

## 영역별 컨셉

### 캐릭터 & IP

- [[wiki/concepts/캐릭터 IP 가이드]]
- [[wiki/concepts/캐릭터 디자인 원칙]]
- [[wiki/concepts/라이선스 표준 조항]]

### 제작

- [[wiki/concepts/웹 제작 워크플로우]]
- [[wiki/concepts/상세페이지 제작 표준]]
- [[wiki/concepts/품질 체크리스트]]

### 마케팅 & 영업

- [[wiki/concepts/마케팅 채널 전략]]
- [[wiki/concepts/리드 발굴 플레이북]]
- [[wiki/concepts/제안서 표준 구조]]

### 운영

- [[wiki/concepts/회계 운영 원칙]]
- [[wiki/concepts/토큰 예산 정책]]

## 엔티티 인덱스

- [[wiki/entities/README|클라이언트 / 프로젝트 / 캐릭터 entity 목록]]

## 커뮤니티 (Graphify Leiden 클러스터)

자동 생성 영역. 첫 빌드 후 `wiki/communities/` 아래에 클러스터별 페이지가 생깁니다.

- [[wiki/communities/README]]

## 누적 질의 응답

- [[wiki/queries/README|에이전트 질의 결과 아카이브]]

## 메타

- [[README]] — vault 전체 안내
- [[frontmatter-standard]] — YAML 스키마
- [[bootstrap-checklist]] — 셋업 절차
- [[skills/README]] — 에이전트 명단

## Stale 경고

`last_reviewed`가 6개월 이상 된 페이지는 [[skills/knowledge-manager/SKILL|Knowledge Manager]]가 매주 수요일 점검합니다.
